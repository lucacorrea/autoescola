...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['reenviar_token'])) {
    ...
    $sql = "UPDATE login_aluno SET reset_token = :token, reset_token_validade = DATE_ADD(NOW(), INTERVAL 15 MINUTE) WHERE email = :email";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['token'])) {
    ...
    $sql = "SELECT reset_token, reset_token_validade FROM login_aluno WHERE email = :email";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':email', $email, PDO::PARAM_STR);
    $stmt->execute();
...
<head>
    ...
    <link rel="stylesheet" href="./painel/css/token.css">
</head>
<body>
    <section class="login">
        <div class="container">
