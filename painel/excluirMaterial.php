...
if (isset($_GET['id'])) {
    ...
    // Prepara a consulta para excluir o material
    $sql = "DELETE FROM materiais_estudo WHERE id = :id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);

    // Tenta executar a consulta
    if ($stmt->execute()) {
        // Se bem-sucedido, redireciona para a página de configurações ou onde desejar
        echo "<script>alert('Material excluído com sucesso!'); window.location.href='guiaEstudo.php';</script>";
    } else {
        echo "<script>alert('Erro ao excluir o material.'); window.location.href='guiaEstudo.php';</script>";
    }
} else {
    echo "<script>alert('ID não fornecido.'); window.location.href='guiaEstudo.php';</script>";
}
?>
